...
import 'package:major_project/services/utils/utils.dart';
...
class _MapPageState extends State<MapPage> {
  ...
  void setPostMarkers() {
    final markerPopup = Provider.of<MarkerPopupModel>(context, listen: false);
    final postsList = Provider.of<List<Post>>(context);
    if (postsList != null) {
      postsList.forEach((v) => {
            _markers.add(Marker(
              markerId: MarkerId(v.postid.toString()),
              position: v.location,
              icon: MarkerBitmapper.instance.messageIcon,
              onTap: () {
                markerPopup.updatePopup(
                  context,
                  mapController,
                  v.location,
                  _infoWindowWidth,
                ...
                markerPopup.updatePost(v);
                markerPopup.updateProfile(null);
                markerPopup.updateVisibility(true);
                markerPopup.rebuild();
                print(markerPopup);
              },
            ))
      ...
      profileList.forEach((v) => {
            _markers.add(Marker(
              ...
              onTap: () {
                ...
                markerPopup.updatePost(null);
                markerPopup.updateProfile(v);
                markerPopup.updateVisibility(true);
                markerPopup.rebuild();
                print(markerPopup);
              },
  ...
  void _setCovidOverlay(List<List<Location>> locations, List<String> cases) {
    ...
    whatever.forEach((e) {
      _setCircle(
          LatLng(e.first[0].latitude, e.first[0].longitude), int.parse(e.last));
    });
  }

  void _setCircle(LatLng location, int cases) {
    //if i have to put markers or the whole widget thing to get labels again i might as well use markers
    ...
    _circles.add(Circle(
      ...
      fillColor: Colors.red[600].withOpacity(0.4),
  ...
  Widget build(BuildContext context) {
    ...
    _setCovidOverlay(
        CovidDB.instance.tableCoordinates, CovidDB.instance.caseList);
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(FlutterI18n.translate(context, "mappage.map")),
      ),
      body: Container(
        //consume markers here
        child: Consumer<MarkerPopupModel>(
            ...
            return Stack(
              children: <Widget>[
                ...
                Positioned(
                  ...
                  top: 0,
                  child: Visibility(
                    visible: markerPopup.showInfoWindow,
                    child: (markerPopup.post == null &&
                                markerPopup.profile == null ||
                            !markerPopup.showInfoWindow)
                        ? Container()
                        : Container(
                            margin: EdgeInsets.only(
                              left: markerPopup.leftMargin,
                              top: markerPopup.topMargin,
                            ),
                            //start popupmodel
                            child: (markerPopup.profile == null)
                                ? Container(
                                    ...
                                    child: ListTile(
                                      leading: ClipOval(
                                        ...
                                        imageUrl: markerPopup.post.userImgURL,
                                      ...
                                      title: Text(markerPopup.post.body),
                                      subtitle: Text(markerPopup.post.username),
                                    ))
                                : Container(
                                    height: 100,
                                    width: 250,
                                    child: InkWell(
                                        child: ProfileCard(markerPopup.profile),
                                        onTap: () async => {
                                              print("tapped"),
                                              print(sessionId),
                                              if (sessionId == null)
          ...
          child: Positioned(
            child: GoogleMap(
              onTap: (position) {
                if (markerPopup.showInfoWindow) {
                  markerPopup.updateVisibility(false);
                  markerPopup.rebuild();
                }
              },
              onCameraMove: (position) {
                if (markerPopup.post != null) {
                  markerPopup.updatePopup(
                    context,
                    mapController,
                    markerPopup.post.location,
                    _infoWindowWidth,
                    _popupOffset,
                  );
                  markerPopup.rebuild();
                }
                if (markerPopup.profile != null) {
                  markerPopup.updatePopup(
                    context,
                    mapController,
                    markerPopup.profile.location,
                    _infoWindowWidth,
                    _popupOffset,
                  );
                  markerPopup.rebuild();
