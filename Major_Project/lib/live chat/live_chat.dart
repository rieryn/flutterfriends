            ...
            return Scaffold(
                ...
                body: StreamBuilder(
                    ...
                    builder: (context, snapshot) {
                      ...
                      return OuterRing(
                          deviceDirection: snapshot.data.direction,
...
class _OuterRingState extends State<OuterRing> {
  ...
  void initState() {
    ...
    textBubbles.add(new TextBubble(
        "north",
        Position(
            latitude: widget.location.latitude + 1,
            longitude: widget.location.longitude)));
    textBubbles.add(new TextBubble(
        "northEast",
        Position(
            latitude: widget.location.latitude + 1,
            longitude: widget.location.longitude + 0.0001)));
    textBubbles.add(new TextBubble(
        "further northEast",
        Position(
            latitude: widget.location.latitude + 1,
            longitude: widget.location.longitude + 0.0002)));
    textBubbles.add(new TextBubble(
        "east",
        Position(
            latitude: widget.location.latitude,
            longitude: widget.location.longitude + 1)));
    textBubbles.add(new TextBubble(
        "south",
        Position(
            latitude: widget.location.latitude - 1,
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: CustomMultiChildLayout(
          delegate: OuterRingLayoutDelegate(
              textBubbles, widget.deviceDirection, widget.location),
...
class OuterRingLayoutDelegate extends MultiChildLayoutDelegate {
  ...
  OuterRingLayoutDelegate(
      this.textBubbles, double deviceDirection, this.location) {
  ...
  void performLayout(Size size) {
    ...
    for (int i = 0; i < textBubbles.length; i++) {
      if (hasChild(i)) {
        ...
        double textBubbleDirection = (GeolocatorPlatform.instance
                    .bearingBetween(
                        location.latitude,
                        location.longitude,
                        textBubbles[i].location.latitude,
                        textBubbles[i].location.longitude) /
                360) *
            2 *
            math.pi;
        ...
        if (textBubbleDirection > deviceDirection) {
          textBubbleDirection =
              2 * math.pi - (textBubbleDirection - deviceDirection);
        } else {
        ...
        textBubbles[i].bufferPosition = getDisplayLocation(
                textBubbles[i].direction, childSize.height, childSize.width)
            .dLocation;
    ...
    for (int i = 0; i < textBubbles.length; i++) {
      if (hasChild(i)) {
        ...
        List<TextBubble> overlapping = cFGrid
            .checkOverlap(
                textBubbles[i].displaySize, textBubbles[i].bufferPosition)
            .toList();

        Map<TextBubble, Offset> originalPositions =
            new Map<TextBubble, Offset>();
        ...
        while ((overlapping.length > 0) && (!doesNotFit)) {
          ...
          if ((overlapping[0].direction - overlapping.last.direction).abs() >
              math.pi) {
            averageAngle = (averageAngle + math.pi) % (2 * math.pi);
          }
          DisplayLocation groupLocation =
              getDisplayLocation(averageAngle, 0, 0);
          ...
          for (int i = 0; i < overlapping.length - 1; i++) {
            if (groupLocation.side == 1) {
              b[overlapping.length + 1 + i][0] =
                  overlapping[overlapping.length - 1].displaySize.height / 2 +
                      childSpacing;
              for (int j = 0; j < overlapping.length - 2 - i; j++) {
                b[overlapping.length + 1 + i][0] +=
                    overlapping[1 + j].displaySize.height + childSpacing;
              }
              b[overlapping.length + 1 + i][0] +=
                  overlapping[i].displaySize.height / 2;
            } else if (groupLocation.side == 3) {
              b[overlapping.length + 1 + i][0] =
                  overlapping[0].displaySize.height / 2 + childSpacing;
              for (int j = 0; j < overlapping.length - 2 - i; j++) {
                b[overlapping.length + 1 + i][0] +=
                    overlapping[overlapping.length - 2 - j].displaySize.height +
                        childSpacing;
              }
              b[overlapping.length + 1 + i][0] +=
                  overlapping[overlapping.length - 1 - i].displaySize.height /
                      2;
            } else if (groupLocation.side == 2) {
              b[overlapping.length + 1 + i][0] =
                  overlapping[overlapping.length - 1].displaySize.width / 2 +
                      childSpacing;
              for (int j = 0; j < overlapping.length - 2 - i; j++) {
                b[overlapping.length + 1 + i][0] +=
                    overlapping[1 + j].displaySize.width + childSpacing;
              }
              b[overlapping.length + 1 + i][0] +=
                  overlapping[i].displaySize.width / 2;
            } else {
              b[overlapping.length + 1 + i][0] =
                  overlapping[0].displaySize.width / 2 + childSpacing;
              for (int j = 0; j < overlapping.length - 2 - i; j++) {
                b[overlapping.length + 1 + i][0] +=
                    overlapping[overlapping.length - 2 - j].displaySize.width +
                        childSpacing;
              }
              b[overlapping.length + 1 + i][0] +=
                  overlapping[overlapping.length - 1 - i].displaySize.width / 2;
            }
          }
          ...
          if (groupLocation.side == 1) {
            for (int i = 0; i < overlapping.length; i++) {
              ...
              if ((overflowStart == -1) &&
                  (calcPos[i][0] + (overlapping[i].displaySize.height / 2) >
                      displayAreaHeight)) {
                overflowStart = i;
              }
            }
            ...
            for (int i = 0; i < overlapping.length; i++) {
              ...
              if ((overflowStart == -1) &&
                  (calcPos[i][0] + (overlapping[i].displaySize.width / 2) >
                      displayAreaWidth)) {
                overflowStart = i;
              }
            }
            ...
            for (int i = 0; i < overlapping.length; i++) {
              if (calcPos[i][0] -
                      (overlapping[overlapping.length - 1 - i]
                              .displaySize
                              .height /
                          2) <
                  0) {
...

          {
            ...
            if (groupLocation.side == 1) {
              for (int i = start; i < end; i++) {
                overlapping[i].bufferPosition = Offset(
                    0, calcPos[i][0] - (overlapping[i].displaySize.height / 2));
              }
            } else if (groupLocation.side == 2) {
              for (int i = start; i < end; i++) {
                overlapping[i].bufferPosition = Offset(
                    calcPos[i][0] - (overlapping[i].displaySize.width / 2),
                    displayAreaHeight - overlapping[i].displaySize.height);
              }
            } else if (groupLocation.side == 3) {
              for (int i = start; i < end; i++) {
                overlapping[i].bufferPosition = Offset(
                    displayAreaWidth - overlapping[i].displaySize.width,
                    calcPos[overlapping.length - 1 - i][0] -
                        (overlapping[i].displaySize.height / 2));
              }
            } else {
              for (int i = start; i < end; i++) {
            ...
            if (groupLocation.side == 1) {
              for (int i = underflowStart; i >= 0; i--) {
                if (i == overlapping.length - 1) {
                  ...
                  overlapping[i].bufferPosition = Offset(
                      overlapping[i + 1].bufferPosition.dx +
                          overlapping[i + 1].displaySize.width +
                          childSpacing,
                      0);
                }
              }
              ...
              for (int i = underflowStart; i >= 0; i--) {
                if (i == overlapping.length - 1) {
                  overlapping[i].bufferPosition = Offset(
                      0, displayAreaHeight - overlapping[i].displaySize.height);
                } else {
                  overlapping[i].bufferPosition = Offset(
                      0,
                      overlapping[i + 1].bufferPosition.dy -
                          overlapping[i].displaySize.height -
                          childSpacing);
                }
              }
            } else if (groupLocation.side == 3) {
              for (int i = underflowStart; i >= 0; i--) {
                if (i == overlapping.length - 1) {
                  ...
                  overlapping[i].bufferPosition = Offset(
                      overlapping[i + 1].bufferPosition.dx -
                          overlapping[i].displaySize.width -
                          childSpacing,
                      displayAreaHeight - overlapping[i].displaySize.height);
                }
              }
              ...
              for (int i = underflowStart; i >= 0; i--) {
                if (i == overlapping.length - 1) {
                  overlapping[i].bufferPosition = Offset(
                      displayAreaWidth - overlapping[i].displaySize.width, 0);
                }
                overlapping[i].bufferPosition = Offset(
                    displayAreaWidth - overlapping[i].displaySize.width,
                    overlapping[i + 1].bufferPosition.dy +
                        overlapping[i + 1].displaySize.height +
                        childSpacing);
              }
            }
          }
          ...
          if (overflowStart != -1) {
            if (groupLocation.side == 1) {
              for (int i = overflowStart; i < overlapping.length; i++) {
                if (i == 0) {
                  overlapping[0].bufferPosition = Offset(
                      0, displayAreaHeight - overlapping[0].displaySize.height);
                } else {
                  overlapping[i].bufferPosition = Offset(
                      overlapping[i - 1].bufferPosition.dx +
                          overlapping[i - 1].displaySize.width +
              ...
              for (int i = overflowStart; i < overlapping.length; i++) {
                if (i == 0) {
                  overlapping[0].bufferPosition = Offset(
                      displayAreaWidth - overlapping[0].displaySize.width,
                      displayAreaHeight - overlapping[0].displaySize.height);
                } else {
                  overlapping[i].bufferPosition = Offset(
                      displayAreaWidth - overlapping[i].displaySize.width,
                      overlapping[i - 1].bufferPosition.dy -
              ...
              for (int i = overflowStart; i < overlapping.length; i++) {
                if (i == 0) {
                  overlapping[0].bufferPosition = Offset(
                      displayAreaWidth - overlapping[0].displaySize.width, 0);
                } else {
                  overlapping[i].bufferPosition = Offset(
                      overlapping[i - 1].bufferPosition.dx -
                          overlapping[i].displaySize.width -
          ...
          for (int i = 0; i < overlapping.length; i++) {
            nowOverlapping.addAll(cFGrid.checkOverlap(
                overlapping[i].displaySize, overlapping[i].bufferPosition));
          }

          if (nowOverlapping.length != 0) {
            //redo position calculations and include the new overlapping textBubbles
            originalPositions.forEach((key, value) {
              key.bufferPosition = value;
            });
            overlapping.addAll(nowOverlapping);
            for (int i = 0; i < nowOverlapping.length; i++) {
              originalPositions[nowOverlapping.elementAt(i)] =
                  nowOverlapping.elementAt(i).bufferPosition;
        ...
        if ((overlapping.length == 0) &&
            (textBubbles[i].bufferPosition != textBubbles[i].displayPosition)) {
  ...
  DisplayLocation getDisplayLocation(
      double direction, double textBubbleHeight, double textBubbleWidth) {
    double x;
    double y;
    int side;
    ...
    if ((direction >= ((math.pi / 2) - aspectAngle)) &&
      ...
      x = 0;
      y = (((displayAreaHeight / 2) -
                  ((displayAreaWidth / 2) *
                      math.tan((math.pi / 2) - direction))) /
              displayAreaHeight) *
      ...
      x = (((displayAreaWidth / 2) -
              ...
              displayAreaWidth) *
          (displayAreaWidth - textBubbleWidth);
      side = 2;
...
class chatFieldGrid {
  ...
  Set<TextBubble> checkOverlap(Size textBubbleSize, Offset textBubblePosition) {
    ...
    int endRow =
        (textBubblePosition.dy + textBubbleSize.height) ~/ gridBlockHeight;
    if (endRow == gridRows) {
      endRow--;
    }
    int startColumn = textBubblePosition.dx ~/ gridBlockWidth;
    int endColumn =
        (textBubblePosition.dx + textBubbleSize.width) ~/ gridBlockWidth;
        ...
        for (int i = 0; i < grid[column][row].length; i++) {
          ...
          if (!overlapping.contains(existingTextBubble)) {
            ...
            List<Offset> existingVertices = {
              ...
              Offset(
                  existingTextBubble.bufferPosition.dx +
  ...
  void add(TextBubble textBubble) {
    ...
    int endRow =
        (textBubble.bufferPosition.dy + textBubble.displaySize.height) ~/
            gridBlockHeight;
    if (endRow == gridRows) {
      endRow--;
    }
    ...
    int endColumn =
        (textBubble.bufferPosition.dx + textBubble.displaySize.width) ~/
            gridBlockWidth;
    if (endColumn == gridColumns) {
      endColumn--;
    }
